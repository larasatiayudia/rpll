				...
				<?php
					if(isset($_POST['preview'])){ // Jika user menekan tombol Preview pada form 
						...
						foreach($sheet as $row){ 
							...
							$nilai = $row['E']; // Ambil data alamat
							
							// Cek jika semua data tidak diisi
							if(empty($nis) && empty($nama) && empty($jenis_kelamin) && empty($alamat) && empty($nilai))
							...
							if($numrow > 1){
								...
								$nilai_td = ( ! empty($nilai))? "" : " style='background: #E07171;'"; // Jika Nilai kosong, beri warna merah
								
								// Jika salah satu data ada yang kosong
								if(empty($nis) or empty($nama) or empty($jenis_kelamin) or empty($alamat) or empty($nilai)){
